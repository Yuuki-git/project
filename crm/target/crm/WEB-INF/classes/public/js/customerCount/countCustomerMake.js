layui.use(['table','layer'],function() {
    ...
    // 获取折线图数据
    $.get("customer/countCustomerMake", {}, function (data) {
        ...
        if (data.code == 0) {
            ...
            for (let i = 0; i < data.data.length; i++) {
                name.push(data.data[i].customerName);
                value.push(data.data[i].customerValue);
                list.push({
                    "name": data.data[i].customerName,
                    "value": data.data[i].customerValue
                });
            }
            // 折柱图
