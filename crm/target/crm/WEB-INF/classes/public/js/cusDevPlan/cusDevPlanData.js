layui.use(['table','layer'],function(){
    ...
    if (devResult == 0 || devResult == 1) {
        tableIns = table.render({
            elem: '#cusDevPlanList',
            url : 'cus_dev_plan/list?saleChanceId='+$("input[name='id']").val(),
            cellMinWidth : 95,
            page : true,
            height : "full-125",
            limits : [5,10,15,20],
    ...
    table.on('toolbar(cusDevPlans)', function(obj){
        switch(obj.event){
            ...
            case 'add':
                // 开启子窗口
                openCusDevPlanDialog("客户开发计划 - 添加计划项", "cus_dev_plan/toAddAndUpdatePage?sId="+$("[name='id']").val());
                break;
            // 批量删除
            case 'del':
                // 获取被选中的数据
    ...
    table.on('tool(cusDevPlans)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'edit':
                ...
                openCusDevPlanDialog("客户开发计划 - 修改计划项", "cus_dev_plan/toAddAndUpdatePage?sId=" + $("[name='id']").val() + "&id=" + obj.data.id);
                break;
            // 删除
            case 'del':
