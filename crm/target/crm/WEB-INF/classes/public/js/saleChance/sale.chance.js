layui.use(['table','layer'],function(){
    ...
    function formatterState(state){
        if(state==0) {
            ...
            return "<div style='color: #ff0000'>未知</div>";
        }
    }
    ...
    table.on('toolbar(saleChances)', function(obj){
        switch(obj.event){
            ...
            case 'add':
                // 开启子窗口
                openSaleChanceDialog("销售机会管理 - 添加营销机会", "sale_chance/toSaleChancePage");
                break;
            // 批量删除事件
            case 'del':
                // 获取被选中的数据
    ...
    table.on('tool(saleChances)', function(obj){
        ...
        switch(obj.event){
            // 修改事件
            case 'edit':
                openSaleChanceDialog("销售机会管理 - 修改营销机会", "sale_chance/toSaleChancePage?id=" + obj.data.id);
                break;
            // 删除事件
            case 'del':
