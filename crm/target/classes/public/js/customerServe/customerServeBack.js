layui.use(['table','layer','form'],function() {
    ...
    var tableIns = table.render({
        ...
        url: 'customerServe/customerServesList?createPeople=' + $.cookie("userName"), // 访问数据的地址
        cellMinWidth : 95,
        page : true, // 开启分页
        height : "full-125",
        limits : [10,15,20,25],
    ...
    function formatterBtn(customerServe) {
        ...
        if (customerServe.state == 'fw_004') {
            ...
            return "<a class='layui-btn layui-btn-xs layui-btn-warm' lay-event='pass'>审核通过</a>" +
                ...
                "<a class='layui-btn layui-btn-xs layui-btn-danger' lay-event='edit'>反馈</a>";
        } else {
            // 删除按钮不可用
            return "<a class='layui-btn layui-btn-xs layui-btn-warm layui-btn-disabled'>审核通过</a>" +
                ...
                "<a class='layui-btn layui-btn-xs layui-btn-danger layui-btn-disabled'>反馈</a>";
        }
    }
    ...
    table.on('tool(customerServe)', function(obj){
        switch(obj.event){
            case 'edit':
                // 修改
                openCustomerServeDialog("服务管理 - 服务反馈", "customerServe/toCustomerServeBackPage?id=" + obj.data.id);
                break;
            // 服务通过
            case 'pass':
                if (!obj.data.serviceProceResult) {
                    layer.msg("请填写反馈结果", {icon: 5});
                    break;
                }
                if (!obj.data.myd) {
                    layer.msg("请填写满意度", {icon: 5});
                    break;
                }
