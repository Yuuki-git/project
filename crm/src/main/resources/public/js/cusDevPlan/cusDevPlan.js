layui.use(['table','layer'],function() {
    ...
    var tableIns = table.render({
        ...
        url : 'sale_chance/list?flag=1', // 访问数据的地址
        cellMinWidth : 95,
        page : true, // 开启分页
        height : "full-125",
        limits : [10,15,20,25],
    ...
    function formatterDevResult(value){
        if(value == 0) {
            return "<div style='color: #ffa500'>未开发</div>";
        } else if(value==1) {
    ...
    table.on('toolbar(cusDevPlans)', function(obj){
        switch(obj.event){
            ...
            case 'add':
                // 开启子窗口
                openSaleChanceDialog("客户开发计划 - 添加营销机会", "sale_chance/toSaleChance");
                break;
            // 批量删除事件
            case 'del':
                // 获取被选中的数据
    ...
    table.on('tool(cusDevPlans)', function(obj){
        ...
        switch(obj.event){
            // 开发
            case 'dev':
                openSaleChanceDialog("客户开发计划 - 计划项数据开发", "cus_dev_plan/toCusDevPlan?id=" + obj.data.id);
                break;
            // 详情
            case 'info':
                openSaleChanceDialog("客户开发计划 - 计划项数据维护", "cus_dev_plan/toCusDevPlan?id=" + obj.data.id);
                break;
        };
    });
