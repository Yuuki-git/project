layui.use(['table','layer','form'],function() {
    ...
    var tableIns = table.render({
        ...
        url: 'customerServe/customerServesList?assigner=' + $.cookie("userName"), // 访问数据的地址
        cellMinWidth : 95,
        page : true, // 开启分页
        height : "full-125",
        limits : [10,15,20,25],
    ...
    function formatterBtn(customerServe) {
        ...
        if (customerServe.state == 'fw_003') {
            ...
            return "<a class='layui-btn layui-btn-xs layui-btn-warm' lay-event='pass'>审核通过</a>" +
                ...
                "<a class='layui-btn layui-btn-xs layui-btn-danger' lay-event='edit'>处理</a>";
        } else {
            // 按钮不可用
            return "<a class='layui-btn layui-btn-xs layui-btn-warm layui-btn-disabled'>审核通过</a>" +
                ...
                "<a class='layui-btn layui-btn-xs layui-btn-danger layui-btn-disabled'>处理</a>";
        }
    }
    ...
    table.on('tool(customerServe)', function(obj){
        switch(obj.event){
            case 'edit':
                ...
                layer.prompt({
                    title: '服务处理 - 服务处理意见', formType: 2, value: obj.data.serviceProce, yes: function (index, layero) {
                        // 校验参数
                        let serviceProceStr = layero.find(".layui-layer-input").val();
                        if (!serviceProceStr) {
                            layer.msg("服务处理意见不能为空", {icon: 5});
                            return false;
                        }
                        // 数据加载遮罩层
                        ...
                        $.post(
                            "customerServe/updateCustomerServe",
                            {
                                id: obj.data.id,
                                state: "fw_003",
                                serviceProce: serviceProceStr,
                                serviceProcePeople: $.cookie("userName")
                            },
                            function (data) {
                                // 关闭数据加载遮罩层
                                layer.close(msg);
            ...
            case 'pass':
                ...
                if (!obj.data.serviceProce) {
                    layer.msg("请先填写服务处理意见", {icon: 5});
                    break;
                }
